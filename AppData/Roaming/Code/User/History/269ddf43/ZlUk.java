...
public class Main {
    ...
    static long countMultiples(long l, long r, long d) {
        return r / d - (l - 1) / d;
    }
    ...
    static long countBadNumbers(long l, long r) {
        // The bad primes
        long[] primes = {2, 3, 5, 7};
        int n = primes.length;
        long bad = 0;

        // Loop over all non-empty subsets of {2, 3, 5, 7}
        for (int mask = 1; mask < (1 << n); mask++) {
            ...
            for (int i = 0; i < n; i++) {
                if ((mask & (1 << i)) != 0) {
                    currLcm = lcm(currLcm, primes[i]);
                    bits++;
                }
            }

            long multiples = countMultiples(l, r, currLcm);
